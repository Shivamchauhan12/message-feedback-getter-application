...
const Page = () => {
  ...
  const [issendeingMessage, setIsSendingMessage] = useState(false)
...
const [chatInput, setChatInput] = useState("");
  ...
  async function getSuggestedMessages(prompt?: string) {
    try {
      const res = await axios.post("/api/gemenai-model", { test: prompt })
      return res;
    } catch (error) {
      const axiosError = error as AxiosError<ApiResponse>
      toast(axiosError.response?.data.message);
  ...
  async function getMessages() {
    ...
    if (data?.data.data) {
      const newMessages = data.data.data.split("||")
  ...
  const handleSendChat = async () => {
    ...
    try {
      const res = await axios.post("/api/gemenai-model", { test: userMsg });
      const botReply = res.data?.data || "Sorry, I couldn't understand that.";
        ...
        <div className="bg-neutral-900 p-8 rounded-2xl shadow-2xl">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              >
                {issendeingMessage ? "Sending Message..." : "Send Message"}
              </Button>
            </form>
          </Form>
        </div>
